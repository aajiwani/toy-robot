...
describe("Command: Move", () => {
	describe("Move towards East", () => {
		it("Should move from (1, 2) to (2, 2) given west direction", () => {
			let result = Commands.Move.Execute(new CommandTupple(1, 2, Direction.EAST));
	...
	describe("Move towards West", () => {
		it("Should move from (1, 2) to (0, 2) given west direction", () => {
			let result = Commands.Move.Execute(new CommandTupple(1, 2, Direction.WEST));
			expect(result).to.be.a("CommandTupple");
			expect(result)
				.to.have.property("X")
				.eq(0);
	...
	describe("Move towards North", () => {
		it("Should move from (1, 2) to (1, 3) given west direction", () => {
			let result = Commands.Move.Execute(new CommandTupple(1, 2, Direction.NORTH));
			expect(result).to.be.a("CommandTupple");
			expect(result)
				.to.have.property("X")
				.eq(1);
	...
	describe("Move towards South", () => {
		it("Should move from (1, 2) to (1, 1) given west direction", () => {
			let result = Commands.Move.Execute(new CommandTupple(1, 2, Direction.SOUTH));
			expect(result).to.be.a("CommandTupple");
			expect(result)
				.to.have.property("X")
				.eq(1);
